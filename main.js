document.addEventListener("DOMContentLoaded", function() {
    ...
    searchButton.addEventListener('click', () => {
        ...
        if (flightData[searchTerm]) {
            ...
            showNotification(`Flight ${searchTerm} not found. Try AB123, AB124, or QFA7550`, true);
        }
    });
  ...
  fetch(url, { headers })
    ...
    .then(data => {
      ...
      results.forEach(item => {
        ...
        const image = get('image', 'https://upload.wikimedia.org/wikipedia/commons/6/65/No-Image-Placeholder.svg');
        ...
        if (!airplanes[name]) {
          airplanes[name] = {
            ...
            speed: speed,
            range: range,
            engineVariants: new Set(), // Store unique engine types here
            capacity: capacity,
            image: image,
            yearBuilt: yearBuilt,
          };
...
window.addEventListener('scroll', () => {
  ...
  // If the user is near the bottom of the page, load more data
