...
const airplaneIconRotated = (angle) => L.divIcon({
  className: '',
  html: `<img src="../plane.png" style="width:24px; transform: rotate(${angle}deg); transition: transform 1s linear;">`,
  iconSize: [24, 24],
  iconAnchor: [12, 12]
});
...
function getNearestAirport(lat, lon) {
  return fetch(`https://nominatim.openstreetmap.org/reverse?format=json&lat=${lat}&lon=${lon}&zoom=10`)
    .then(res => res.json())
    .then(data => data.address?.airport || data.address?.city || data.address?.town || data.display_name || 'Unknown')
    .catch(() => 'Unknown');
}
...
function updateFlightInfoBox({ callsign, origin_country, lat, lon, velocity, altitude, eta, distanceKm, duration, destination, type }) {
  ...
  infoPanel.innerHTML = `
    ...
    <div class="flight-row"><span class="label">ETA:</span><span class="value">${eta}</span></div>
...
async function updateSelectedFlight(lat, lon, angle, speed, altitude, callsign, origin_country) {
  const heading = angle;
  const travelTime = 3600;
  const distance = speed * travelTime;
  const destLat = lat + (Math.cos(heading * Math.PI / 180) * distance / 111320);
  const destLon = lon + (Math.sin(heading * Math.PI / 180) * distance / (40075000 * Math.cos(lat * Math.PI / 180) / 360));

  const distanceKm = (distance / 1000).toFixed(2);
  const { eta, duration } = calculateETA(distance, speed);
  const nearest = await getNearestAirport(destLat, destLon);

  updateFlightInfoBox({ callsign, origin_country, lat, lon, altitude, velocity: speed, eta, distanceKm, duration, destination: nearest, type: 'Airliner' });
...
async function fetchLiveAircraft() {
  try {
    ...
    const filtered = result.states.filter(s => s[6] && s[5] && s[9] > 10).slice(0, MAX_PLANES);
    ...
    for (const state of filtered) {
      ...
      const marker = L.marker([lat, lon], { icon: airplaneIconRotated(angle) }).addTo(map);
      marker.bindPopup(`✈️ ${callsign}<br>Alt: ${Math.round(altitude)} m`);

      marker.on('click', () => {
        selectedPlaneCallsign = callsign;
        updateSelectedFlight(lat, lon, angle, speed, altitude, callsign, state[2]);
        map.setView([lat, lon], 7);
      });

      aircraftMarkers.set(callsign, marker);

      if (callsign === selectedPlaneCallsign) {
        updateSelectedFlight(lat, lon, angle, speed, altitude, callsign, state[2]);
        marker.setLatLng([lat, lon]);
        marker.setIcon(airplaneIconRotated(angle));
        map.setView([lat, lon]);
      }
    }
  } catch (err) {
...
fetchLiveAircraft();
setInterval(fetchLiveAircraft, 10000);
